...
#   (2) ind{0 <= x : sum(x) = r}.
# Therefore SumLargest(r) now is only correct when r = 1. To make SumLargest correct
# we should extend IndSimplex to allow for that additional bound in its definition,
